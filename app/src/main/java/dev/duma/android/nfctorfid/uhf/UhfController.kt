...
class UhfController(private val context: Context) {
    ...
    private fun onServiceReady() {
        ...
        _state.value =
            if (model <= MODEL_NONE) ReaderState.NoReader else ReaderState.Ready(model)
    }

    /** Suspends until the reader is ready and returns the detected model. */
    suspend fun awaitReady(timeoutMs: Long = 7_000): Int {
        (_state.value as? ReaderState.Ready)?.let { return it.model }
        connect()
        val settled = try {
        ...
        return (settled as? ReaderState.Ready)?.model
